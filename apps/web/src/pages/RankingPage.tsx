...
export default function RankingPage() {
  ...
  const tierStyle = previewRank ? (TIER_STYLES[previewRank.current.name] ?? TIER_STYLES['beginner']) : null;
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Calculate your strength tier per muscle group</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="glass rounded-2xl p-5 space-y-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">Rank Calculator</p>
          <form onSubmit={calculate} className="space-y-3">
            {/* Muscle group pills */}
            <div>
              <p className="text-xs text-slate-500 mb-2">Muscle Group</p>
              <div className="flex flex-wrap gap-1.5">
                {MUSCLE_GROUPS.map((mg) => (
                  <button
                    key={mg}
                    type="button"
                    onClick={() => setForm({ ...form, muscleGroup: mg })}
                    className={`text-xs px-3 py-1.5 rounded-full font-semibold capitalize transition-all ${
                      form.muscleGroup === mg
                        ? muscleColor(mg) + ' ring-1 ring-current'
            ...
            <div className="grid grid-cols-2 gap-2">
              {([
                ...
                ['Reps', 'reps', '1', '100'],
                ['Workouts/week', 'workoutsPerWeek', '0', '14'],
                ['Streak (days)', 'streakDays', '0', '365'],
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl glass px-3 py-2.5 text-sm text-white placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-orange-500/40 text-center"
            ...
            <div>
              <div className="flex justify-between mb-1">
                ...
                <p className="text-[11px] text-orange-400 font-bold">{Math.round(Number(form.volumeProgression) * 100)}%</p>
              </div>
              <input
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 rounded-xl gradient-orange glow-orange text-white text-sm font-bold hover:scale-[1.02] active:scale-[0.97] transition-transform disabled:opacity-60"
            >
          ...
          <div className="glass rounded-2xl p-5 space-y-6">
            ...
            <div className="flex flex-col items-center gap-4 py-2">
              ...
              <div className="text-center">
                ...
                <p className="text-slate-500 text-sm">Score: <span className="text-white font-bold">{previewScore?.toFixed(0)}</span></p>
                {previewRank.next && (
              ...
              {(['beginner', 'bronze', 'silver', 'gold', 'platinum', 'diamond', 'elite'] as const).map((tier) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-semibold capitalize ${style.badge}`}>{tier}</span>
                    {isActive && <span className="ml-auto text-[10px] font-bold" style={{ color: style.color }}>YOU ARE HERE</span>}
                  </div>
                );
              })}
            </div>
            ...
            <button
              onClick={saveRanking}
              disabled={saving}
              className="w-full py-2.5 rounded-xl gradient-green glow-green text-white text-sm font-bold hover:scale-[1.02] active:scale-[0.97] transition-transform disabled:opacity-60"
            >
              {saving ? 'Saving…' : saved ? '✔ Saved!' : 'Save to My Rankings'}
            </button>
          </div>
        ) : (
          <div className="glass rounded-2xl p-5 flex flex-col items-center justify-center text-center min-h-64 gap-3">
            <span className="text-5xl">🏋️</span>
            <p className="text-sm text-slate-500">Fill in your stats and hit<br/><span className="text-orange-400 font-semibold">Calculate Rank</span> to see your tier</p>
          </div>
        )}
      </div>
      ...
      {myRankings.length > 0 && (
        <div className="glass rounded-2xl p-5 space-y-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">My Saved Rankings</p>
              ...
              return (
                <div key={r.id} className="glass rounded-xl p-4 space-y-2">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs font-bold text-orange-400">{Math.round(Number(r.consistency_factor) * 100)}%</span>
                  </div>
